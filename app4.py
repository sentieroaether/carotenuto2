...
def compila_tabella_esistente(doc, df_combinato):
    try:
        ...
        for i, row in enumerate(df_combinato.itertuples(), 1):  # Partiamo dalla seconda riga, indice 1 (la prima è l'intestazione)
            ...
            pod_value = row.pod
            if isinstance(pod_value, str):
                cells[6].text = pod_value  # Se è una stringa, la usiamo così com'è
            else:
                cells[6].text = str(int(float(pod_value)))  # Se è un numero, lo convertiamo correttamente
...
def genera_documento_word(dati, df_combinato, template_path="decreto.docx"):
    ...
    # Debug per il campo "ragione sociale"
    if 'ragione_sociale' not in dati:
        st.error("Il campo 'ragione sociale' non è presente nei dati!")
    else:
        st.write(f"Ragione sociale trovata: {dati['ragione_sociale']}")
    ...
    placeholders = {
        ...
        "{partita_iva}": str(dati.get('partita_iva', '')).replace('nan', ''),
        ...
        "{data_generazione}": data_generazione,  # Aggiungi la data di generazione
        "{provincia_residenza}": str(dati.get('provincia_residenza', '')).replace('nan', ''),
        # Controlla se il campo 'pod' è numerico o alfanumerico
        "{pod}": str(dati.get('pod', '')).replace(',', '.'),  # Se è alfanumerico, usalo così com'è
